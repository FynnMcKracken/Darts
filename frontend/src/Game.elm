...
import Html exposing (Html, button, div, h1, h5, main_, table, tbody, td, text, th, thead, tr)
import Html.Attributes exposing (attribute, class, classList, disabled, id, style)
...
nextPlayer : Cmd Message
nextPlayer =
    sendJsonMessage <| Encode.object [ ( "NextPlayer", Encode.object [] ) ]


missHit : Cmd Message
missHit =
    sendJsonMessage <| Encode.object [ ( "MissHit", Encode.object [] ) ]


closeGame : Cmd Message
closeGame =
    sendJsonMessage <| Encode.object [ ( "CloseGame", Encode.object [] ) ]
...
standardGameDecoder : Decoder Model
standardGameDecoder =
    Decode.field "Standard" <| gameDecoder Standard


cricketGameDecoder : Decoder Model
cricketGameDecoder =
    Decode.field "Cricket" <| gameDecoder Cricket


gameDecoder : GameMode -> Decoder Model
gameDecoder gameMode =
    Decode.map4 Model
        (Decode.succeed gameMode)
        ...
        , div [ class "row mb-4" ]
            [ div [ class "col" ]
                [ table [ class "table table-bordered table-hover" ]
                    ...
                    , tbody [] ([] ++ List.map (renderPlayer model.gameMode) model.players)
                    ]
                ]
            ]
        , div [ class "row mt-4" ]
            [ div [ class "col" ]
                [ button [ class "btn btn-outline-secondary game-button", disabled (not model.running), onClick NextPlayer ] [ text "Next player" ]
                , button [ class "btn btn-outline-danger", disabled (not model.running), onClick MissHit ] [ text "Missed hit" ]
                ]
            , div [ class "col text-right" ]
                [ button [ class "btn btn-outline-danger game-button", disabled (not model.running), attribute "data-target" "#modal-close-game", attribute "data-toggle" "modal" ] [ text "Close game" ]
                ]
            ]
        , div [ class "row mb-2" ]
           ...
           , div [ class "modal fade", id "modal-close-game", attribute "role" "dialog" ]
               [ div [ class "modal-dialog" ]
                   [ div [ class "modal-content" ]
                       ...
                       , div [ class "modal-body" ]
                           [ text "Do you want to close the current game?"
                           ]
                       , div [ class "modal-footer" ]
                           [ button [ class "btn btn-secondary", attribute "data-dismiss" "modal" ] [ text "Cancel" ]
                           , button [ class "btn btn-danger", attribute "data-dismiss" "modal", onClick CloseGame ] [ text "Close game" ]
                           ]
                       ]
                   ]
               ]
...
renderPlayer : GameMode -> Player -> Html Message
renderPlayer gameMode player =
...
renderCricketScore : List ( String, Int ) -> List (Html Message)
...
renderCricketPoints : ( String, Int ) -> Html Message
...
cricketMarker : Int -> Html Message
cricketMarker state =
...
segment : Model -> String -> String -> String -> Html Message
...
circleSegment : Model -> String -> String -> String -> String -> String -> Html Message
